...
const getOrFindById = async <T>(
  context: HookContext, 
  makeParams?: (params: Params, context: HookContext) => Promise<Params>,
  ...
  if (context.id === null) {
    const method = (!options || options.skipHooks) ? "_find" : "find";
    ...
    const method = (!options || options.skipHooks) ? "_get" : "get";
