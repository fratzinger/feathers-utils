...
import { setResultEmpty } from "../../src";
...
const paramsAdapterPaginate = { adapter: { paginate: { default: 10, max: 50 } } };

describe("util - setResultEmpty", function() {
      ...
      ["before", "after"].forEach(type => {
        [paginatedService, nonPaginatedService].forEach(service => {
          [paramsPaginateFalse, paramsAdapterPaginate].forEach(params => {
            ...
            assert.deepStrictEqual(context.result, 123, `result is not changed. '${type}:${method}': '${service}' - '${params}'`);
          });
        });
      });
    });
  ...
  describe("find", function() {
    it("sets paginated result", function() {
      ...
      combos.forEach(({ service, params }, i) => {
        const { result } = setResultEmpty({ service, params, method: "find" } as any as HookContext);
        assert.deepStrictEqual(result, { total: 0, skip: 0, limit: 0, data: [] }, `'${i}': result is paginated empty`);
      });
    });
  ...
  describe("get", function() {
    it("sets result to null", function() {
      ...
      combos.forEach(({ service, params }, i) => {
        const { result } = setResultEmpty({ service, params, method: "get" } as any as HookContext);
  ...
  describe("create", function() {
    it("sets result to null for single data", function() {
      ...
      combos.forEach(({ service, params }, i) => {
        const { result } = setResultEmpty({ service, params, method: "create", data: { id: 1 } } as any as HookContext);
        assert.deepStrictEqual(result, null, `'${i}': result is null`);
      });
    });
    ...
    it("sets result to empty array for array data", function() {
      ...
      combos.forEach(({ service, params }, i) => {
        const { result } = setResultEmpty({ service, params, method: "create", data: [{ id: 1 }], type: "before" } as any as HookContext);
        assert.deepStrictEqual(result, [], `'${i}': result is empty array`);
      });
    });
  });
  ...
  describe("update", function() {
    it("sets result to null", function() {
      ...
      combos.forEach(({ service, params }, i) => {
        const { result } = setResultEmpty({ service, params, method: "update", id: 1 } as any as HookContext);
        assert.deepStrictEqual(result, null, `'${i}': result is null`);
      });
    });
  });
  ...
  describe("patch", function() {
    it("sets result to null for id: 1", function() {
      ...
      combos.forEach(({ service, params }, i) => {
        const { result } = setResultEmpty({ service, params, method: "patch", id: 1 } as any as HookContext);
        assert.deepStrictEqual(result, null, `'${i}': result is null`);
      });
    });
    ...
    it("sets result to empty array for id: null", function() {
      ...
      combos.forEach(({ service, params }, i) => {
        const { result } = setResultEmpty({ service, params, method: "patch", id: null } as any as HookContext);
        assert.deepStrictEqual(result, [], `'${i}': result is empty array`);
      });
    });
  });
  ...
  describe("remove", function() {
    it("sets result to null for id: 1", function() {
      ...
      combos.forEach(({ service, params }, i) => {
        const { result } = setResultEmpty({ service, params, method: "remove", id: 1 } as any as HookContext);
        assert.deepStrictEqual(result, null, `'${i}': result is null`);
      });
    });
    ...
    it("sets result to empty array for id: null", function() {
      ...
      combos.forEach(({ service, params }, i) => {
        const { result } = setResultEmpty({ service, params, method: "remove", id: null } as any as HookContext);
        assert.deepStrictEqual(result, [], `'${i}': result is empty array`);
      });
    });
  });
