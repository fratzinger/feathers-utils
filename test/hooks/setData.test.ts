    ...
    it("does not overwrite userId for single item by predicate", function() {
      ...
      Object.keys(methodsByType).forEach(type => {
        methodsByType[type].forEach(method => {
          ...
          //@ts-ignore
          const result = setData("params.user.id", "userId", { overwrite: (item) => item.userId == null })(context);
          assert.strictEqual(result[dataOrResult].userId, 2, `'${type}/${method}': ${dataOrResult} has 'userId:2'`);
        });
      });
    });
    ...
    it("predicate based on context", function() {
      ...
      Object.keys(methodsByType).forEach(type => {
        methodsByType[type].forEach(method => {
          ...
          //@ts-ignore
          const result = setData("params.user.id", "userId", { overwrite: (item, context) => context.type === "before" })(context);
          if (type === "before") {
            assert.strictEqual(result[dataOrResult].userId, 1, `'${type}/${method}': ${dataOrResult} has 'userId:1'`);
          } else {
            assert.strictEqual(result[dataOrResult].userId, 2, `'${type}/${method}': ${dataOrResult} has 'userId:2'`);
